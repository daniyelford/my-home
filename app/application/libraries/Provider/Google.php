...
// 		$url_info=$this->CI->Include_model->resive_data_only(strtolower($url));
		
// 		var_dump($url_info);
// 		die();
		$user = json_decode(file_get_contents($url), true);
// 		var_dump($user);
		if(!empty($user) && !empty($user['id']) && !empty($user['name']))
    		return array(
    			'uid' => $user['id'],
    			'nickname' => url_title($user['name'], '_', true),
    			'name' => $user['name'],
    			'first_name' => (!empty($user['given_name'])?$user['given_name']:''),
    			'last_name' => (!empty($user['family_name'])?$user['family_name']:''),
    			'email' => $user['email'],
    			'location' => null,
