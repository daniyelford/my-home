		...
		switch ($this->method)
		{
			...
			case 'POST':
				...
				*/
				...
				// var_dump($contex);
				// if(!empty($context)) $response = @file_get_contents($url, false, $context);
				// if(!empty($context)) $response = $this->CI->Include_model->send_data_and_resive_data($url,$context);
                // if($response && !empty($response)) $return = json_decode($response, true);
                // else $return='0';
			break;
