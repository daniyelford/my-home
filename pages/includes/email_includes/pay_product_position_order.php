    ...
    <div class="div">
        ...
        <p>
            <?php if(!empty($data['me'])){ ?>
                <p>
                    <img src="<?= (!empty($data['me']['image'])?$data['me']['image']:base_url('assets/svg/user/user.svg')) ?>">
                    <span>
                        <?= (!empty($data['me']['name'])?$data['me']['name']:'').''.(!empty($data['me']['family'])?$data['me']['family']:'') ?>
                    </span>
                </p>
            <?php } ?>
            <p>مشتری محصول رزرو شده در جایگاه را پرداخت کرده</p>
    	</p>
    </div>
    <?php if(!empty($data)){ ?>
        <div>
            <?php if(!empty($data['price'])){ ?>
                <h2>مبلغ پرداختی: <?= number_format($data['price']) ?></h2>
            <?php } ?>
            <article>
    		    <?= (!empty($text)?$text:'') ?>
            </article>
        ...
        <div class="div">
            ...
            <?php } if(!empty($data['product'])){ ?>
                <p>
                    <img src="<?= base_url('assets/'.(!empty($data['product']['icon'])?'svg/product/'.$data['product']['icon']:(!empty($data['product']['qr_code'])?'qrcode/'.$data['product']['qr_code']:'svg/product/product.svg'))) ?>">
                    <h2>
                        <?= (!empty($data['product']['title'])?$data['product']['title']:'') ?>
                    </h2>
                    <h3>
                        <?= (!empty($data['product']['description'])?$data['product']['description']:'') ?>
                    </h3>
                    <h5><?php $date=new JDF(); echo $date->jdate('Y/m/d H:i',time()); ?></h5>
                </p>
                <br>
