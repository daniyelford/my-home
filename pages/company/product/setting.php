    ...
    <div class="row row-sm mt-3">
    			    ...
    			    <div class="pl-0">
    				    <div class="main-profile-overview">
    					    <div class="main-img-user profile-user">
    						    <img alt="product profile" src="<?= base_url('assets/svg/product/'.(!empty($data['info']['icon'])?$data['info']['icon']:'product.svg')) ?>">
    						</div>
    						<br>
    						<label class="main-content-label tx-13 mg-b-20"><?= (!empty($data['info']['title'])?$data['info']['title']:'') ?></label>
    						<hr class="mg-y-10">
							...
							<div class="row row-sm mt-2">
                        	    <div class="col-md-7 mx-auto">
                                    <img class="w-100 rounded-10" onclick="downloadImage(this);" src="<?= base_url('assets/qrcode/'.(!empty($data['info']['qr_code'])?$data['info']['qr_code']:'tes.png')) ?>" alt="product qrcode">
                                </div>
                            </div>
    					</div>
    				</div>
    	...
    	<div class="col-lg-8">
    		<div class="row row-sm">
    		    <div class="col-12 text-center mb-1" onclick="productElementsToolsForManager(this,'reserve');">
    		    ...
    		    <div class="col-12 text-center mb-1" onclick="productElementsToolsForManager(this,'keys');">
			    ...
			    <div class="show-div-setting col-12">
    				...
    				<div class="chat <?= (!empty($_GET['action']) && $_GET['action']=='chat'?'':'d-none') ?>" id="chatmodelproduct<?= intval($p_id) ?>">
    				    <?= $data['chat'] ?>
    				</div>
    				...
    				<div class="keys <?= (!empty($_GET['action']) && $_GET['action']=='keys'?'':'d-none') ?>">
    				    <?= $data['key_value'] ?>
    				</div>
                					        ...
                					        <div class="card">
        				                        ...
        				                        <div class="card-body pt-1">
        				                            <div style="max-height:400px;overflow-y:auto;overflow-x:hidden;">
                                    				    <?php $num=0; foreach(array_reverse($data['reserve']) as $res){
                                    				        if(!empty($res) && !empty($res['position_user_info'])){ $num++;
                                                    	    ...
                                                    	    <div class="alert alert-danger rounded-10 text-center p-3">
                                                    	        شما هیچ سفارشی برای این محصول ندارید
                                                    	    </div>
                                                    	<?php } ?>
        				                            </div>
        				                        </div>
    ...
    <script>
        $(function (){
            <?php if(!empty($_GET['action']) && $_GET['action']=='map'){ ?>
                mapMarkerChangeLocationImage('product',true,0,0,0,<?= $p_id ?>);
            <?php } ?>
        });
        function checkBackUrlFunction(){
            processAjaxData(document.title,$('#content').html(),baseUrl('company_product_setting'));
        }
        ...
        function productElementsToolsForManager(el,type){
            ...
            processAjaxData(document.title,$('#content').html(),baseUrl('company_product_setting?action='+type));
        	return true;    
        }
        function showProductMapManagerTool(){
            processAjaxData(document.title,$('#content').html(),baseUrl('company_product_setting?action=map'));
            mapMarkerChangeLocationImage('product',true,0,0,0,<?= $p_id ?>);
        }
    </script>
<?php } ?>