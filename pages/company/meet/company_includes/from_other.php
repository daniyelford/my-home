<div class="col-xl-9 col-lg-8 col-md-12 col-sm-12" id="from-other">
    <?php $date=new JDF();
    $other=(!empty($data) && !empty($data['other']) && is_array($data['other'])?$data['other']:[]);
    if(!empty($other)){ ?>
        <div>
		    <h6 class="card-title mb-1">
			    درخواست های دیگران
			</h6>
			<p class="text-muted card-sub-title">
                درخواست های دیگران از من برای اجرای جلسه
			</p>
		</div>
		<div aria-multiselectable="true" class="accordion" id="accordion" role="tablist"  style="overflow-y:auto;overflow-x:hidden;height:400px;">
		    <?php $check_idss=[];
			foreach(array_reverse($other) as $aa){ 
                if(!empty($aa) ){
                    if(!empty($aa['info']['id']) && intval($aa['info']['id'])>0 && !in_array(intval($aa['info']['id']),$check_idss)){
                        $check_idss[]=intval($aa['info']['id']); ?>
                        <div class="card mb-0">
            			    <div class="card-header" id="heading<?= $aa['info']['id'] ?>" role="tab">
            				    <a class="text-center" aria-controls="collapse<?=  $aa['info']['id'] ?>" aria-expanded="true" data-toggle="collapse" href="#collapse<?=  $aa['info']['id'] ?>">
            					    <?= (!empty($aa['info']['title'])?$aa['info']['title']:'') ?>
            					</a>
            				</div>
            				<div aria-labelledby="heading<?= $aa['info']['id'] ?>" class="collapse" data-parent="#accordion" id="collapse<?=  $aa['info']['id'] ?>" role="tabpanel">
                                <?php foreach(array_reverse($other) as $a){
                                    if(!empty($a) && !empty($a['info']['id']) && intval($a['info']['id'])>0 && intval($aa['info']['id'])===intval($a['info']['id'])){ ?> 
                                        <div class="card-body">
                                            <div class="email-media shadow-light p-2 rounded-10">
                                                <div class="mt-0 d-sm-flex">
                                				    <img class="ml-2 rounded-circle avatar-md" src="<?= (!empty($a['from_user_info']['image'])?$a['from_user_info']['image']:base_url('assets/svg/user/user.svg')) ?>" alt="user image">
                                					<div class="media-body">
                                					    <div class="float-left d-flex fs-15">
                                						    <span class="mr-3 mt-1 <?= (!empty($a['status']) && intval($a['status'])==1?'text-success':'text-danger') ?>">
                                    						    <?= (!empty($a['status']) && intval($a['status'])==1?'تایید شده':'در انتظار تایید') ?>
                                							</span>
                                							<small class="mr-3 mt-1">
                                							    <a onclick="showMeetDetail(this);" class="show">
                                								    <i class="fe fe-clipboard tx-16" title="مشاهده جزئیات"></i>
                                								</a>
                                								<a onclick="hideMeetDetail(this);" class=" hide d-none">
                                								    <i class="si si-logout tx-16" title="بستن جزئیات"></i>
                                								</a>
                                							</small>
                                						</div>
                                						<div class="media-title font-weight-bold mt-3">
                                						    <span class="">
                                    						    <?= (!empty($a['from_user_info']['name'])?$a['from_user_info']['name']:'') ?> 
                                							</span>
                                							<span class="">
                                							    <?= (!empty($a['from_user_info']['family'])?$a['from_user_info']['family']:'') ?>
                                							</span>
                                						</div>
                                						<?php if(!empty($a['from_user_info']['phone'])){ ?>
                                    					    <a href="tel:<?= $a['from_user_info']['phone'] ?>">
                                        					    <small class="mr-1">
                                        						    <i class="la la-phone tx-15" title="تماس"></i>
                                        						</small>
                                    						</a>
                                						<?php } ?>
                                						<span class="mr-3">
                                						    <?= (!empty($a['time'])?$date->jdate('Y/m/d H:i',strtotime($a['time'])):'') ?>
                                						</span>
                                					</div>
                                				</div>
                                				<div class="eamil-body mt-2 d-none">
                                                    <p class="">
                                                        دلیل نیاز به جلسه:
                                                        <?= (!empty($a['info']['title'])?$a['info']['title']:'') ?>
                                                    </p>
                                    				<p class="">
                                    				    توضیحات نسبی :
                                						<?= (!empty($a['info']['description'])?$a['info']['description']:'ندارد') ?>
                                    				</p>
                                    				<p class="tx-10-f text-center">
                                    				    زمان فراخوان :
                                						<?= (!empty($a['info']['run_time'])?$date->jdate('Y/m/d H:i',strtotime($a['info']['run_time'])):'در دست بررسی') ?>
                                    				</p>
                                    				<hr>
                                    				<?php if(empty($a['status']) || intval($a['status'])==0){ ?>
                                        			    <div class="row options">
                                        				    <?php if(!empty($a['info']['run_time'])){ ?>
                                            				    <div class="col-6">
                                            					    <a class="btn btn-danger-gradient rounded-10 p-3 w-100" onclick="changeTime(<?= (!empty($a['info']['id']) && intval($a['info']['id'])>0?intval($a['info']['id']):0) ?>,<?= (!empty($a['meet_user_id']) && intval($a['meet_user_id'])>0?intval($a['meet_user_id']):0) ?>,2);">
                                            						    تغییر زمان
                                            					    </a>
                                            					</div>
                                            					<div class="col-6">
                                            					    <a class="btn btn-success-gradient rounded-10 p-3 w-100" onclick="acceptMeet(<?= (!empty($a['meet_user_id']) && intval($a['meet_user_id'])>0?intval($a['meet_user_id']):0) ?>);">
                                            						    تایید زمان  
                                            					    </a>
                                            					</div>
                                            				<?php }else{?>
                                            				    <div class="col-12">
                                            					    <a class="btn btn-success-gradient rounded-10 p-3 w-100" onclick="changeTime(<?= (!empty($a['info']['id']) && intval($a['info']['id'])>0?intval($a['info']['id']):0) ?>,<?= (!empty($a['meet_user_id']) && intval($a['meet_user_id'])>0?intval($a['meet_user_id']):0) ?>,2);">
                                            						    تعیین زمان
                                            					    </a>
                                            					</div>
                                        					<?php } ?>
                                        				</div>
                                    				<?php }elseif(!empty($a['status']) && intval($a['status'])==1){
                                    				    if(empty($a['info']['status']) || intval($a['info']['status'])==0){ ?>
                                        				    <div class="alert alert-primary rounded-10 text-center p-4">
                                        					    زمان اجرا تایید شده و در وضعیت انتظار برای اجرا شدن می باشد
                                        				    </div>
                                        			    <?php }else{ ?>
                                    			            <div class="alert alert-success rounded-10 text-center p-4">
                                					            اجرا شده
                                					        </div>
                            					        <?php }
                            					    }elseif(!empty($a['status']) && intval($a['status'])==2){ ?>
                            					        <div class="alert alert-warning rounded-10 text-center p-4">
                                    					    درانتظار تایید زمان بندی مجدد شما توسط درخواست کننده
                                    				    </div>
                                    			    <?php } ?>
                                					<hr>
                            						<div class="email-attch">
                        								<?php if(!empty($a['status']) && intval($a['status'])==1){ ?>
                                							<p class="">
                                    						    نتیجه ی جلسه:
                                    						    <?= (!empty($a['info']['result'])?$a['info']['result']:'ندارد') ?>
                                            			    </p>
                                        			    <?php } ?>
                            						</div>
                                				</div>
                        			        </div>
                                        </div>
                                    <?php }
                                } ?>
            				</div>
            			</div>
                <?php }
                } 
            } ?>
		</div>
	<?php }else{ ?>
        <div class="alert alert-danger rounded-10 text-center p-5">
            هیچ درخواستی از شما برای تشکیل جلسه ای در کسب کارتان وجود ندارد
        </div>
    <?php } ?>
</div>